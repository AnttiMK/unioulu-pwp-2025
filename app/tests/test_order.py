...
class OrderViewTests(TestCase):
    def setUp(self):
        ...
        self.menu_item = MenuItem.objects.create(name="Test Item", description="Test Description", price=10.0)
...
# Tests for GET requests
    ...
    def test_get_orders_by_status(self):
        ...
        response = self.client.get(reverse("Get by status", args=['pending']))
        self.assertEqual(response.status_code, 200)
        self.assertIn('order_count', response.json())

    def test_get_order_by_id(self):
        # Test that the get_by_id view returns the correct order
        response = self.client.get(reverse("Order by id", args=[self.order.id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_orders_by_user(self):
        ...
        response = self.client.get(reverse("Orders by user", args=[self.user.name]))
        self.assertEqual(response.status_code, 200)
...
#Tests for POST, UPDATE and DELETE requests
    ...
    def test_create_order_with_incorrect_status(self):
        ...
        response = self.client.post(reverse("Create order"), json.dumps(data), content_type="application/json")
        self.assertNotEqual(response.status_code, 201, "Creating an order with an incorrect status type is not allowed!")
    ...
    def test_update_order_invalid_json(self):
        ...
        response = self.client.put(reverse("Update order", args=[self.order.id]), "invalid json", content_type="application/json")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.content.decode(), "Invalid JSON format.")

    def test_delete_order(self):
        # Test that an order can be deleted
        response = self.client.delete(reverse("Delete order", args=[self.order.id]))
        self.assertEqual(response.status_code, 204)


# Tests for the correct request types for GET functions
    def test_post_get_all_orders(self):
        # Test that the get_all view does not allow POST requests
        response = self.client.post(reverse("All orders"))
        self.assertEqual(response.status_code, 405)

    def test_post_get_orders_by_status(self):
        # Test that the get_by_status view does not allow POST requests
        response = self.client.post(reverse("Get by status", args=['pending']))
        self.assertEqual(response.status_code, 405)

    def test_post_get_order_by_id(self):
        # Test that the get_by_id view does not allow POST requests
        response = self.client.post(reverse("Order by id", args=[self.order.id]))
        self.assertEqual(response.status_code, 405)

    def test_post_get_orders_by_user(self):
        # Test that the get_by_user view does not allow POST requests
        response = self.client.post(reverse("Orders by user", args=[self.user.name]))
        self.assertEqual(response.status_code, 405)

# Tests for the correct request types for POST, UPDATE and DELETE functions

    def test_get_create_order(self):
        response = self.client.get(reverse("Create order"))
        self.assertEqual(response.status_code, 405)

    def test_get_update_order(self):
        response = self.client.get(reverse("Update order", args=[self.order.id]))
        self.assertEqual(response.status_code, 405)

    def test_get_delete_order(self):
        response = self.client.get(reverse("Delete order", args=[self.order.id]))
        self.assertEqual(response.status_code, 405)
...
# Tests for invalid requests
    ...
    def test_get_orders_by_invalid_user(self):
        ...
        response = self.client.get(reverse("Orders by user", args=["Invalid User"]))
        self.assertEqual(response.status_code, 404)
    ...
    def test_delete_order_with_invalid_id(self):
        ...
        response = self.client.delete(reverse("Delete order", args=[9999]))
        self.assertEqual(response.status_code, 404)
