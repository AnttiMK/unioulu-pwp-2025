...
class Order(models.Model):
    ...
    def all_items(self):
        return ", ".join(f"{item.name} - {item.price}€" for item in self.menu_items.all()) or "No items"
    ...
    def clean(self):
        ...
        if self.table:
            if self.number_of_people < self.table.min_people:
                raise ValidationError(f"Too few people for this table. Minimum required: {self.table.min_people}.")
            if self.number_of_people > self.table.max_people:
                raise ValidationError(f"Too many people for this table. Maximum allowed: {self.table.max_people}.")
